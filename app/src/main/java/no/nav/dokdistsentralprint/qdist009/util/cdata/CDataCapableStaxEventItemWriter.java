 ...
 * <p/>
 * Copy from spring-batch-infrastructure:2.2.2.RELEASE, changing line 452 in the code, using
 * CDataCapableNoStartEndDocumentStreamWriter instead of NoStartEndDocumentStreamWriter
 *
 * @author Peter Zozom
 * @author Robert Kasanicky
 * @author Michael Minella
 */
public class CDataCapableStaxEventItemWriter<T> extends AbstractItemStreamItemWriter<T> implements
		ResourceAwareItemWriterItemStream<T>, InitializingBean {

	private static final Log log = LogFactory.getLog(CDataCapableStaxEventItemWriter.class);
	...
	public CDataCapableStaxEventItemWriter() {
		setExecutionContextName(ClassUtils.getShortName(CDataCapableStaxEventItemWriter.class));
	}
	...
	private void open(long position, boolean restarted) {
		...
		try {
			...
			eventWriter = new CDataCapableNoStartEndDocumentStreamWriter(delegateEventWriter);
